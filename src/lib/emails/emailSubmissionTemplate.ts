...
export function generateSubmissionConfirmation(data: SubmissionConfirmationData): string {
  ...
  const callText = callTitle ? `(${callTitle} call)` : '';

  return `<div style="font-family: Arial, sans-serif; line-height: 1.4; color: #333; max-width: 600px;">
    Dear ${formattedNames},
    
    Thank you for the submission of your ${typeDisplay} ${titleText} to Haven Spec Magazine${callText}. Your submission is now in our queue, and it will be reviewed by our editorial team.
